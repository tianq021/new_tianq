...
`maxFiles` 不能当成固定母版值直接复制。它必须和业务流程一致。

```text
单文件流程：
- 如果代码中使用 files[0]、first、第一份文件、主要票据，maxFiles 必须为 1。
- 开场白必须写“请上传 1 份文件”。
- 判断器和错误回复也必须按单文件说明。

多文件流程：
- 只有用户明确要求批量处理时，maxFiles 才能大于 1。
- 代码节点必须遍历 files。
- AI 输出必须是数组，例如 receipts[]。
- 总结节点必须按多份文件汇总。
- 禁止 maxFiles > 1 但实际只处理 files[0]。
```
...
6. 文件上传
   ...
   - maxFiles 必须和实际处理逻辑一致：单文件流程 maxFiles=1；多文件流程必须遍历 files 并输出数组。
...
18. maxFiles 是否和代码节点、AI 提示词、开场白、最终回复一致。
19. 如果 maxFiles > 1，是否真的遍历 files 并输出多文件数组结果。
```
...
## 错误 6：maxFiles 与实际处理逻辑不一致

错误：

```text
chatConfig.fileSelectConfig.maxFiles = 5 或 10，
但代码节点只取 files[0]，AI 提示词也只处理一份票据。
```

正确：

```text
如果只处理一份文件：maxFiles = 1。
如果允许多份文件：代码必须遍历 files，AI 输出 receipts[]，总结按多份汇总。
```
