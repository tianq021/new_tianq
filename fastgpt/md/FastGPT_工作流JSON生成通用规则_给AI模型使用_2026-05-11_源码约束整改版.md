...
`maxFiles` 是业务规则，不是固定模板值，不能直接照抄示例。

生成工作流时必须先判断实际处理逻辑：

```text
1. 如果代码节点、AI 提示词或后续流程只处理第一份文件，例如使用 files[0] / first / 主要票据：
   - chatConfig.fileSelectConfig.maxFiles 必须设为 1；
   - 开场白必须写“请上传 1 份文件”；
   - 错误提示也必须按单文件说明；
   - 不允许写“最多 5 个文件 / 最多 10 个文件”。

2. 如果 maxFiles > 1：
   - 代码节点必须遍历 files；
   - AI 提取结果必须设计为数组，例如 receipts[]；
   - 总结节点必须按多份文件汇总；
   - 不能只取 files[0]；
   - 开场白必须说明“可上传多份文件”。
...
3. 自检时必须同时检查：
   ...
   - code 节点是否遍历 files；
   - AI 提示词是单文件还是多文件；
   - welcomeText / answerNode 提示是否与 maxFiles 一致。
```

默认建议：

```text
没有明确要求批量处理时，一律按单文件模板生成，maxFiles = 1。
只有用户明确要求“批量、多份、多张、多文件”时，才允许 maxFiles > 1，并且必须同步改成数组处理逻辑。
```
